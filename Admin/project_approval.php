...
<title>项目审批 - <?php echo conf('Name');?> - 后台管理中心</title>
    ...
    <div class="col-lg-12">
      <div class="card">
        <div class="card-toolbar clearfix">
          <form class="pull-right search-bar" method="get" onsubmit="return searchProject()" role="form">
            <div>
              <div class="form-group" style="display: inline-block;float: left;">
              <select name="column" class="form-control"><option value="1">搜索</option><option value="name">名字</option><option value="url">网址</option><option value="type">类型</option></select>
              </div>
          ...
          <div class="toolbar-btn-action">
            <a class="btn btn-primary m-r-5" href="./pset.php?my=add"><i class="mdi mdi-plus"></i> 新增</a>
          </div>
        </div>
...
function listTable(query){
  ...
  if(query == 'start' || typeof query === 'undefined'){
    ...
    history.replaceState({}, null, './project_approval.php?'+query);
  }
  layer.closeAll();
  var ii = layer.load(2, {shade:[0.1,'#fff']});
  $.ajax({
    type : 'GET',
    url : 'project_approval-table.php?'+query,
    dataType : 'html',
    cache : false,
    ...
    success : function(data) {
      if(data.code == 0){
        ...
        layer.confirm('是否显示于首页', {
          ...
          $.ajax({
            type : 'GET',
            url : 'ajax.php?act=setProject&type=Show&id='+id+'&num=1',
            dataType : 'json',
            success : function(data) {
              if(data.code == 0){
                listTable();
